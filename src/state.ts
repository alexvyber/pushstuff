...
import type { ExternalToast, PromiseData, ToastType, ToastToDismiss, TypesOfToast } from "./types"
...
class Observer {
  ...
  promise = <ToastData>(promise: Promise<ToastData> | (() => Promise<ToastData>), data?: PromiseData<ToastData>) => {
    if (!data) {
      return
    }
